...
const (
	outFileName = "placeholder.bin"
	outSize     = "1gb" // 如 512、1kb、500mb、2gb、1.5g（1024 进制）
	sparse      = false // true=仅预分配长度；false=逐块写 0
)

func main() {
	fmt.Println("file-raw — 生成指定大小的占位文件 → exe 同目录/output/")
	if strings.TrimSpace(outFileName) == "" || strings.TrimSpace(outSize) == "" {
		fmt.Fprintln(os.Stderr, "请在 cmd/file-raw/main.go 中填写 outFileName 与 outSize")
		os.Exit(1)
	}
